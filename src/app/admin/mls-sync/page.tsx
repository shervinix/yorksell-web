...
export default function AdminMlsSyncPage() {
  ...
  return (
    <div className="space-y-8">
      <h1 className="text-xl font-semibold text-zinc-900 dark:text-zinc-50">MLS Sync</h1>

      <section className="rounded-2xl border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-900">
        <h2 className="text-sm font-medium text-zinc-500 dark:text-zinc-400">Last run</h2>
        {loading ? (
          <p className="mt-2 text-sm text-zinc-600 dark:text-zinc-400">Loading…</p>
        ) : !state ? (
          <p className="mt-2 text-sm text-zinc-600 dark:text-zinc-400">No sync has run yet.</p>
        ) : (
      ...
      <section className="rounded-2xl border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-900">
        <h2 className="text-sm font-medium text-zinc-500 dark:text-zinc-400">Run sync now</h2>
        <div className="mt-4 flex flex-wrap items-end gap-4">
          <div>
            <label htmlFor="limit" className="block text-xs text-zinc-500">Limit</label>
            <input
              id="limit"
              type="number"
              min={1}
          ...
          <div>
            <label htmlFor="pages" className="block text-xs text-zinc-500">Pages</label>
            <input
              id="pages"
              type="number"
              min={1}
          ...
          <label className="flex items-center gap-2">
            <input
              ...
              className="rounded border-zinc-300"
            />
            <span className="text-sm text-zinc-600 dark:text-zinc-400">Dry run (no DB writes)</span>
          </label>
          <button
            type="button"
            onClick={runSync}
      ...
      <section className="rounded-2xl border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-900">
        <h2 className="text-sm font-medium text-zinc-500 dark:text-zinc-400">Daily sync</h2>
        <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">
          Run DDF sync automatically once per day at the set time (requires Vercel Cron and{" "}
          <code className="rounded bg-zinc-100 px-1 dark:bg-zinc-800">CRON_SECRET</code> in production).
        </p>
        {scheduleLoading ? (
          ...
          <div className="mt-4 space-y-4">
            <label className="flex items-center gap-2">
              <input
                ...
                className="rounded border-zinc-300"
              />
              <span className="text-sm text-zinc-700 dark:text-zinc-300">Enable daily sync</span>
            </label>
            <div className="flex flex-wrap items-end gap-4">
              <div>
                <label htmlFor="schedule-time" className="block text-xs text-zinc-500">Time</label>
                <input
                  id="schedule-time"
                  type="time"
                  value={schedule.time}
                  onChange={(e) => setSchedule((s) => (s ? { ...s, time: e.target.value } : s))}
                  className="mt-1 rounded-lg border border-zinc-200 px-2 py-1.5 text-sm dark:border-zinc-700 dark:bg-zinc-800"
                />
              </div>
              <div>
                <label htmlFor="schedule-tz" className="block text-xs text-zinc-500">Timezone</label>
                <select
                  id="schedule-tz"
            ...
            {schedule.lastScheduledRunAt && (
              <p className="text-sm text-zinc-500 dark:text-zinc-400">
                Last auto-run: {new Date(schedule.lastScheduledRunAt).toLocaleString()}
              </p>
            )}
